...
| `BILI_PERSIST_ZSTD` | 设为 `0` 可禁用会话文件的 zstd 压缩（#1080）。默认情况下，每个未加密的会话文件均以 `BILIZSTD1` 格式写入——即在 JSON 主体之上附加一个小头部（魔数 + 格式版本 + 模式字节），在 Node ≥ 22.15 上以 zstd 压缩存储，在旧版运行时上则原样存储；读取端兼容两种主体格式（优先使用原生 zstd，否则回退至内置的 WASM 实现），因此文件在不同运行时和版本间均可正常读取。与 `BILI_ENCRYPTION_KEY` 相互独立——两者同时生效时，压缩方式会记录在 `BILIENC1` 内部的模式字节中。已有的纯 JSON 文件将在首次升级时一次性迁移：每个文件都会被重新编码并原子性地就地替换（迁移过程中若发生崩溃，下次启动时将自动恢复）。 |
