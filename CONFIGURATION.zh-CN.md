...
- **默认值：** *（v2 起代理模式默认开启（#1179）— 未设置时解析为 `enabled: true`；在任意层级显式设置 `enabled: false` 即可关闭。插件通道保持显式开启：需全局 `enabled: true` 才武装（#1271/#1273））*
- **状态：** ACTIVE（v2 — 代理模式默认开启；插件通道限 anthropic + openai wire，显式开启后生效）
