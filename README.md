...
An opt-in seventh tool, `acp_retrieve` (`compress.ccr.enabled: true` — see [CONFIGURATION.md](CONFIGURATION.md)), backs the **content-addressed message store** (built-in CCR, #1097): oversized tool results are **ID-referenced at arrival instead of force-distilled** — the wire keeps a byte-stable placeholder and the original goes into a per-session content-store envelope (hash-deduped), retrievable on demand via one cheap tool call. Lossless by default: a retrieve not made costs nothing but the call; a detail distilled away by absorb is gone for good. Proxy mode plus plugin lanes on the anthropic + openai wires in v1 (`acp_retrieve` is advertised in the plugin manifest while CCR is enabled, #1271); responses marker/text routes and google stay disarmed because no request-only round-trip channel exists there (silent loss, #1097).
