...
An opt-in fifth tool, `absorb` (`compress.absorb.enabled: true` — see [CONFIGURATION.md](CONFIGURATION.md)), compresses **individual tool results the moment they arrive**: large results (builds, logs, greps) get a forced absorb instruction, the model distills each into a compact summary, and the original pair is hidden from the wire from the next turn on — keeping mid-session pressure lower between fold rounds (#605). A further opt-in layer, `compress.absorb.preCrush.enabled` (#1094), runs a deterministic, model-free denoise in front of that decision: JSON constant-field/identical-row folding (lossless), comment/docstring elision for Python and JS/TS (string- and template-literal safe), and level-classified log selection that keeps every ERROR line, dedupes warnings, and collapses runtime stack frames — so results crushed below the threshold skip the model round-trip entirely, and anything unparseable passes through byte-identical.
