...
8. **Plugin identity across restart / model switch / failed retry (#739)** — the `plugin-conversations` map also holds plain cache-key mappings, so it must NOT be used to infer plugin identity; a mapped conversation keeps its original session and compression state, and recomputed compressible ranges are clamped to the original history boundary. Regression entry points: `tests/subagent-namespace.test.ts` and `tests/preflight-fail-fast.test.ts`.
