...
test("closed stderr pipe: no uncaughtException storm, file-only logging, one warn (#1233)", async () => {
    ...
    fs.writeFileSync(childScript, [
        `import { configureLogger, log, isStreamWriteError } from ${JSON.stringify(loggerPath)};`,
        ...
        `});`,
        `const t0 = Date.now();`,
        `const timer = setInterval(() => log("info", \`tick \${Date.now() - t0}\`), 50);`,
        `setTimeout(() => { clearInterval(timer); process.stdout.write(\`FIRED \${fired}\\n\`); process.exit(0); }, 2500);`,
    ].join("\n"));
    ...
    const code = await new Promise<number | null>((resolve, reject) => {
        const fail = setTimeout(() => reject(new Error("child timed out after pipe close")), 15000);
    ...
    assert.ok(ticks.length >= 20, `file-only logging must keep the durable record (got ${ticks.length} tick lines)`);
