/**
 * Deterministic pre-crush channel (#1094): mechanical size reduction of
 * eligible oversized tool results, applied view-level BEFORE the model-absorb
 * gate. Pure function of (payload, config) — identical bytes on every turn a
 * result is re-sent (prefix-cache stable); raw bytes never leave the proxy;
 * no kernel state is mutated.
 *
 * Strategies (dispatched by content sniffing):
 *  - json: field-variance folding (concept adapted from headroom SmartCrusher,
 *    headroomlabs-ai/headroom, Apache-2.0; implementation original TS).
 *    LOSSLESS BY CONSTRUCTION. Annotation vocabulary (reserved key `__pre_crush`):
 *      {"__pre_crush":"rows","rows":N,"const":{k:v,...},"items":[...]}
 *        array of N rows; every row = `const` merged with its `items` entry
 *        (same order); `items` omitted iff no row has a non-constant field.
 *      {"__pre_crush":"identical-run","count":N,"item":X}
 *        N consecutive identical elements X.
 *    Payloads legitimately containing a `__pre_crush` key cannot be told apart
 *    from crushed output — disable preCrush for them.
 *  - code: structural trimmer (Python, JavaScript/TypeScript; sound for other
 *    // -comment languages). LOSSY: full-line comments, docstrings and
 *    blank-line runs are elided into one marker line (`# [pre-crush: elided N
 *    lines]` / `// [pre-crush: elided N lines]`) and NOT retained. Code lines,
 *    string literals and trailing comments stay byte-exact.
 *  - log: build/test log condenser (ported from headroom LogCompressor,
 *    Apache-2.0): classifies every line (level / summary / stack-trace
 *    membership), keeps errors+fails (first, last, top-scored), deduped
 *    warnings, bounded stack traces (runtime frames collapsed to a marker),
 *    summary lines and ±3 context, caps the rest by score. LOSSY: noise lines
 *    are dropped, not retained. Upstream deviations: no CCR store — the
 *    omission footer carries honest counts instead of a retrieve hash; the
 *    keep budget is the fixed 100-line cap instead of headroom's adaptive
 ...
 * template), low-confidence language detection, or reduction below the
 * configured minimum leave the payload byte-identical. Query-aware relevance
 * selection (headroom layer b) is deliberately not ported: bili re-sends tool
 * results every turn, so the crush must stay query-independent or provider
 * prefix caches churn.
 */
...
export interface PreCrushResolved {
    enabled: boolean;
    minReduction: number;
}

export const PRE_CRUSH_DEFAULT_MIN_REDUCTION = 0.1;

export type PreCrushAwareAbsorb = NonNullable<Config["absorb"]> & { preCrush?: PreCrushResolved };
export type PreCrushAwareConfig = Config & { absorb?: PreCrushAwareAbsorb };

export interface CrushResult {
    text: string;
    strategy: "json" | "code" | "log";
}
...
// Lenient on purpose: file/env config parsing already rejects malformed values
// loudly; this guards hand-built kernel configs and the plugin-metadata path.
export function resolvePreCrush(absorb: unknown): PreCrushResolved | null {
    const pc = (absorb as { preCrush?: unknown } | null | undefined)?.preCrush;
    if (!pc || typeof pc !== "object" || Array.isArray(pc)) return null;
    const p = pc as Record<string, unknown>;
    if (p.enabled !== true) return null;
    const mr = p.minReduction;
    let minReduction = PRE_CRUSH_DEFAULT_MIN_REDUCTION;
    if (typeof mr === "number" && Number.isFinite(mr) && mr > 0 && mr <= 1) {
        minReduction = mr;
    } else if (typeof mr === "string" && /^\d+(?:\.\d+)?%$/.test(mr.trim())) {
        const ratio = Number(mr.trim().slice(0, -1)) / 100;
        if (ratio > 0 && ratio <= 1) minReduction = ratio;
    }
    return { enabled: true, minReduction };
}

export function tryCrush(text: string, minReduction: number, countTokens: (t: string) => number): CrushResult | null {
    const rawTok = countTokens(text);
    if (rawTok <= 0) return null;
    const accept = (cand: string, strategy: CrushResult["strategy"]): CrushResult | null => {
        if ((rawTok - countTokens(cand)) / rawTok < minReduction) return null;
        return { text: cand, strategy };
    };
    try {
        if (text.length === 0 || text.length > MAX_INPUT_CHARS) return null;
        const trimmed = text.trim();
        // Line-based strategies (code/log) drop lines; on structured data that
        // would corrupt meaning, so JSON-shaped payloads get the lossless json
        // strategy only. Confidently-detected source gets code only — a line
        // selector must never silently drop source lines.
        if ((trimmed.startsWith("{") && trimmed.endsWith("}")) || (trimmed.startsWith("[") && trimmed.endsWith("]"))) {
            const json = crushJson(trimmed);
            if (json !== null) {
                const hit = accept(json, "json");
        ...
        if (lang !== null) {
            const code = lang === "python" ? trimPython(text) : trimJsTs(text);
            if (code !== null) {
                const hit = accept(code, "code");
                if (hit !== null) return hit;
            }
            return null;
        }
        const log = crushLog(text);
        if (log !== null) {
            const hit = accept(log, "log");
            if (hit !== null) return hit;
        }
        return null;
    } catch {
...
function canonOf(v: unknown, cache: Map<object, string>): string {
    if (!isPlainObject(v) && !Array.isArray(v)) return JSON.stringify(v) ?? "null";
...
function compactValue(v: unknown, depth: number, stats: FoldStats, cache: Map<object, string>): unknown {
...
function compactArray(arr: unknown[], depth: number, stats: FoldStats, cache: Map<object, string>): unknown {
    const n = arr.length;
    if (n < RUN_MIN && n < HOIST_MIN) return arr.map((e) => compactValue(e, depth + 1, stats, cache));
...
function isRunMarker(v: unknown): v is { [CRUSH_KEY]: "identical-run"; count: number; item: unknown } {
    return isPlainObject(v) && (v as Record<string, unknown>)[CRUSH_KEY] === "identical-run";
}

function buildRunFold(arr: unknown[], depth: number, cache: Map<object, string>): FoldCandidate | null {
    const n = arr.length;
    ...
    const final = segs.map((s) => (isRunMarker(s) ? { ...s, item: compactValue(s.item, depth + 1, inner, cache) } : compactValue(s, depth + 1, inner, cache)));
    return { value: final, len: strLen(final), folds: runs + inner.folds };
}

function buildConstHoist(arr: Record<string, unknown>[], depth: number, cache: Map<object, string>): FoldCandidate | null {
    const n = arr.length;
    const keys: string[] = [];
    ...
    for (const k of keys) {
        const c0 = canonOf(arr[0][k], cache);
        let constant = true;
        for (let i = 1; i < n; i++) {
            if (!(k in arr[i]) || canonOf(arr[i][k], cache) !== c0) {
                constant = false;
                break;
            }
        }
    ...
    for (const k of constKeys) constObj[k] = arr[0][k];
    ...
    const inner: FoldStats = { folds: 0 };
    const env: Record<string, unknown> = { [CRUSH_KEY]: "rows", rows: n, const: compactValue(constObj, depth + 1, inner, cache) };
    if (hasVarying) env.items = items.map((it) => compactValue(it, depth + 1, inner, cache));
...
function detectLanguage(text: string): Lang | null {
    ...
    for (const line of sample) {
        ...
        if (/^\s*(async\s+)?def\s+\w/.test(line) || /^\s*class\s+\w/.test(line)) py += 2;
        ...
        if (/=>/.test(line)) js += 1;
        if (/^\s*(public|private|protected|static|final|void|return)\b/.test(line)) js += 1;
        if (/;\s*$/.test(line) && line.trim().length > 0) js += 1;
        if (/\bself\./.test(line) || /^\s*(elif|except|yield)\b/.test(line)) py += 1;
        if (/\bconsole\.log\b|\brequire\s*\(|module\.exports|\bawait\s/.test(line)) js += 1;
    }
    const MARGIN = 1.5;
...
function trimPython(text: string): string | null {
    ...
    const flush = () => {
        if (pending > 0) {
            emitted.push(`# [pre-crush: elided ${pending} line${pending === 1 ? "" : "s"}]`);
            pending = 0;
        }
    };
...
function findCommentOrTriple(line: string): { kind: "none" | "comment-full" | "triple-mid"; idx: number; quote: string } {
    let i = 0;
    ...
    while (i < line.length) {
        ...
        if (c === "#") {
            return { kind: line.slice(0, i).trim().length === 0 ? "comment-full" : "none", idx: i, quote: "" };
        }
...
function scanTemplateRest(line: string, from: number, st: TmplState): TmplState & { closed: boolean; bail: boolean; consumed: number } {
    let m = st.mode;
    let q = st.quote;
    let d = st.depth;
    let i = from;
    while (i < line.length) {
        ...
        if (m === "expr") {
            ...
            if (c === "`") return { mode: m, quote: q, depth: d, closed: false, bail: true, consumed: 0 };
            if (c === "{") d++;
            else if (c === "}") {
                d--;
                if (d === 0) m = "tpl";
        ...
        if (c === "`") return { mode: "tpl", quote: "", depth: 0, closed: true, bail: false, consumed: i - from + 1 };
        if (c === "$" && line[i + 1] === "{") {
            m = "expr";
            d = 1;
            i += 2;
    ...
    return { mode: m, quote: q, depth: d, closed: false, bail: false, consumed: 0 };
}

type JsLineKind = "code" | "comment-full" | "block-open-only" | "block-open-mid" | "template-open";

function scanJsCodeLine(line: string): { kind: JsLineKind; tmpl?: TmplState; bail?: boolean } {
    let i = 0;
    let inStr: string | null = null;
    while (i < line.length) {
        const c = line[i];
        ...
        if (c === "`") {
            ...
            if (!r.closed) return { kind: "template-open", tmpl: { mode: r.mode, quote: r.quote, depth: r.depth } };
            i += r.consumed;
            continue;
        }
        if (c === "/" && line[i + 1] === "/") {
            return { kind: line.slice(0, i).trim().length === 0 ? "comment-full" : "code" };
        }
        if (c === "/" && line[i + 1] === "*") {
            const close = line.indexOf("*/", i + 2);
            const before = line.slice(0, i).trim().length > 0;
            if (close === -1) return { kind: before ? "block-open-mid" : "block-open-only" };
...
type LogLevelName = "error" | "fail" | "warn" | "info" | "debug" | "trace" | "unknown";
...
function isLogSummaryLine(line: string): boolean {
    ...
    let d = 0;
    while (d < line.length && line.charCodeAt(d) >= 48 && line.charCodeAt(d) <= 57) d++;
    if (d > 0 && line[d] === " ") {
        const rest = line.slice(d + 1);
        if (rest.startsWith("passed") || rest.startsWith("failed") || rest.startsWith("skipped") || rest.startsWith("error") || rest.startsWith("warning")) return true;
    }
    for (const prefix of ["Test ", "Tests ", "Tests:", "Test:", "Suite ", "Suites ", "Suites:", "Suite:"]) {
        if (line.startsWith(prefix)) {
            const m = line.slice(prefix.length).match(/\S/);
            if (m !== null && m[0].charCodeAt(0) >= 48 && m[0].charCodeAt(0) <= 57) return true;
        }
    }
    if (line.startsWith("TOTAL") || line.startsWith("Total") || line.startsWith("Summary")) return true;
    for (const prefix of ["Build", "Compile", "Test"]) {
        if (line.startsWith(prefix) && (line.includes("succeeded") || line.includes("failed") || line.includes("complete"))) return true;
    }
    return false;
}
...
function hasLineColSuffix(s: string): boolean {
    for (let i = 0; i + 1 < s.length; i++) {
        if (s[i] === ":" && isDigitChar(s[i + 1])) {
            ...
            if (j + 1 < s.length && s[j] === ":" && isDigitChar(s[j + 1])) return true;
        }
    }
    return false;
}

type TraceFlavor = "py" | "js" | "java" | "dotnet" | "rust-error" | "rust-backtrace" | "go-panic";
...
function isJavaAtFrame(s: string): boolean {
    ...
    for (const c of body) {
        if (!(isAsciiAlnum(c) || c === "." || c === "_" || c === "$" || c === "/")) return false;
    }
    return true;
}
...
function traceFlavorFor(line: string): TraceFlavor | null {
    ...
    if (t.startsWith("Traceback (most recent call last)") || isPythonFileFrame(t)) return "py";
    ...
    if (isRustPanicOpener(t) || t.startsWith("stack backtrace:") || isRustBacktraceFrame(line)) return "rust-backtrace";
    if (isGoPanicOpener(line)) return "go-panic";
    return null;
}

function traceTerminates(flavor: TraceFlavor, line: string, linesSoFar: number): boolean {
    const t = line.trimStart();
    switch (flavor) {
        case "py": {
...
function scoreLogLine(l: LogLine): number {
    const base = l.level === "error" || l.level === "fail" ? 1.0 : l.level === "warn" ? 0.5 : l.level === "info" || l.level === "unknown" ? 0.1 : l.level === "debug" ? 0.05 : 0.02;
...
function parseLogLines(lines: string[]): LogLine[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const entry: LogLine = { i, content: line, level: classifyLevel(line), isStack: false, isSummary: isLogSummaryLine(line), score: 0 };
        if (active !== null) {
...
function collapseTraceFrames(stack: LogLine[], headFrames: number, appFrames: number): CollapsedTrace {
    const kept: LogLine[] = [];
    ...
    const flushRun = () => {
        if (runStart !== -1) {
            kept.push({ i: runStart, content: `      [... ${runLen} frames collapsed]`, level: "unknown", isStack: true, isSummary: false, score: 0.8 });
            runStart = -1;
            runLen = 0;
        }
    };
...
function selectLogLines(all: LogLine[]): LogLine[] {
    ...
    for (const l of selectWithFirstLast(errors, LOG_MAX_ERRORS)) selected.set(l.i, l);
    for (const l of selectWithFirstLast(fails, LOG_MAX_ERRORS)) selected.set(l.i, l);
    ...
    for (const w of dedupedWarnings.slice(0, LOG_MAX_WARNINGS)) selected.set(w.i, w);
    for (const stack of stacks.slice(0, LOG_MAX_STACK_TRACES)) {
        if (stack.length > LOG_STACK_MAX_LINES) {
            const collapsed = collapseTraceFrames(stack, LOG_TRACE_HEAD_FRAMES, LOG_TRACE_APP_FRAMES);
            for (const i of collapsed.dropped) selected.delete(i);
            for (const l of collapsed.kept.slice(0, LOG_STACK_MAX_LINES)) selected.set(l.i, l);
        } else {
...
function formatLogOutput(selected: LogLine[], all: LogLine[]): string {
    const count = (lv: LogLevelName) => all.reduce((n, l) => (l.level === lv ? n + 1 : n), 0);
...
export function crushLog(text: string): string | null {
    ...
    if (!(classified >= LOG_CLASSIFIED_GATE || traces >= 1 || summaries >= LOG_SUMMARY_GATE || detectLogFormat(lines) !== "generic")) return null;
...
const lastCrushLog = new Map<string, number>();
const CRUSH_LOG_TTL_MS = 120_000;

function logCrushEvent(state: CompressionState, msg: CoreMessage, rawTok: number, newTok: number, strategy: string): void {
    try {
        const ref = refForRaw(state.messageRefs, msg.id);
        const refText = ref && ref !== BLOCKED_REF ? ref : msg.id;
        const key = `${msg.id}:${(msg.text ?? "").length}`;
        const now = Date.now();
        if (lastCrushLog.size > 1000) lastCrushLog.clear();
        const last = lastCrushLog.get(key);
        if (last !== undefined && now - last < CRUSH_LOG_TTL_MS) return;
        lastCrushLog.set(key, now);
        const pct = rawTok > 0 ? Math.round((1 - newTok / rawTok) * 100) : 0;
        loggerLog("info", `[acp-absorb] pre-crushed ${refText} ${rawTok}→${newTok} tok (-${pct}%) (${strategy})`);
    } catch {
        // a logging failure must never break the request pipeline
    }
}

// Runs between hide and prompt in applyAbsorbView: the absorb gate then sees
// post-crush sizes, so results crushed below minToolTokens stop triggering
// model round-trips automatically. Returns the input array unchanged when
// nothing qualifies.
export function applyPreCrush(messages: CoreMessage[], state: CompressionState, config: PreCrushAwareConfig): CoreMessage[] {
    const absorb = config.absorb;
    if (!absorb || absorb.enabled !== true) return messages;
    const pc = resolvePreCrush(absorb);
    if (!pc) return messages;
    const cfg = { ...DEFAULT_ABSORB_CONFIG, ...absorb };
    const latest = collectLatestProtected(messages, config);
    let changed = false;
    const out: CoreMessage[] = [];
    for (const msg of messages) {
        if (!isAbsorbCandidate(msg, config, latest)) {
            out.push(msg);
            continue;
        }
        // processTurn bakes the absorb prompt onto eligible results before this
        // stage runs; crush the payload only and let appendAbsorbPrompts
        // re-decide on the crushed size (it skips marker-bearing texts).
        const text = msg.text ?? "";
        const markerAt = text.indexOf(ABSORB_PROMPT_MARKER);
        const payload = markerAt >= 0 ? text.slice(0, markerAt) : text;
        const rawTok = defaultCountTokens(payload);
        if (rawTok < cfg.minToolTokens) {
            out.push(msg);
            continue;
        }
        const crushed = tryCrush(payload, pc.minReduction, defaultCountTokens);
        if (!crushed) {
            out.push(msg);
            continue;
        }
        changed = true;
        logCrushEvent(state, msg, rawTok, defaultCountTokens(crushed.text), crushed.strategy);
        out.push({ ...msg, text: crushed.text });
    }
